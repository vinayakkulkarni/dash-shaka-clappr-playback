...
class DashShakaPlayback extends HTML5Video {
  ...
  set currentLevel(id) {
    ...
    const isAuto = this._currentLevelId === DEFAULT_LEVEL_AUTO;
    ...
    if (!isAuto) {
      ...
      this.selectTrack(
        this.videoTracks.filter((t) => t.id === this._currentLevelId)[0],
      );
    } else {
  ...
  get dvrEnabled() {
    return (
      this._duration >= this._minDvrSize && this.getPlaybackType() === 'live'
    );
  }
  ...
  get presentationStartTimeAsDate() {
    if (
      !this.shakaPlayerInstance ||
      !this.shakaPlayerInstance.getPresentationStartTimeAsDate()
    )
      return 0;

    return new Date(
      this.shakaPlayerInstance.getPresentationStartTimeAsDate().getTime() +
        this.seekRange.start * 1000,
    );
  }
  ...
  constructor(...args) {
    ...
    this._minDvrSize =
      typeof this.options.shakaMinimumDvrSize === 'undefined'
        ? 60
        : this.options.shakaMinimumDvrSize;
  }
  ...
  get audioTracks() {
    return (
      ...
      this._player
        .getVariantTracks()
        .filter((t) => t.mimeType.startsWith('audio/'))
    );
  }
  ...
  get videoTracks() {
    return (
      ...
      this._player
        .getVariantTracks()
        .filter((t) => t.mimeType.startsWith('video/'))
    );
  }
  ...
  _enableShakaTextTrack(isEnable) {
    ...
    Array.from(this.el.textTracks)
      ...
      .forEach(
        (track) => (track.mode = isEnable === true ? 'showing' : 'hidden'),
      );
  }
  ...
  _startToSendStats() {
    const intervalMs =
      this._options.shakaSendStatsInterval || SEND_STATS_INTERVAL_MS;
  ...
  _onError(err) {
    ...
    const { category, code, severity } =
      error.shakaError.detail || error.shakaError;
  ...
  _onAdaptation() {
    const activeVideo = this.videoTracks.filter((t) => t.active === true)[0];
